...
class TestUserEdit(Basecase):
    def test_edit_just_created_user(self):
        ...
        response3 = MyRequests.put(f"/api/user/{user_id}", headers={"x-csrf-token": token},
                                   cookies={"auth_sid": auth_sid}, data={"firstName": new_name})
        Assertions.assert_code_status(response3, 200)

        # Get
        response4 = MyRequests.get(f"/api/user/{user_id}", headers={"x-csrf-token": token},
    ...
    def test_edit_just_created_user_not_auth(self):
        ...
        # Edit
        new_name = "Changed name"
        response2 = MyRequests.put(f"/api/user/{user_id}", data={"firstName": new_name})
    ...
    def test_edit_just_created_user_auth_as_another_user(self):
        ...
        new_name = "Changed name"
        response4 = MyRequests.put(f"/api/user/{user_id2}", headers={"x-csrf-token": token1},
        ...
        response5 = MyRequests.post("/api/user/login", data=login_data2)
        ...
        response6 = MyRequests.get(f"/api/user/{user_id2}", headers={"x-csrf-token": token2},
    ...
    def test_edit_just_created_user_with_very_short_firstname_auth(self):
        ...
        # Check that firstName wasn't changed
        response4 = MyRequests.get(f"/api/user/{user_id}", headers={"x-csrf-token": token},
                                   cookies={"auth_sid": auth_sid})
