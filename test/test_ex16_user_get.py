...
class TestUserGet(Basecase):
    def test_get_user_details_not_auth(self):
        response = MyRequests.get("/api/user/1")
    ...
    def test_get_user_details_auth_as_same_user(self):
        ...
        response2 = MyRequests.get(f"/api/user/{user_id_from_auth_method}", headers={"x-csrf-token": token},
    ...
    def test_get_user_details_auth_as_another_user(self):
        ...
        auth_sid = self.get_cookies(response1, "auth_sid")
        token = self.get_headers(response1, "x-csrf-token")
        response2 = MyRequests.get("/api/user/1", headers={"x-csrf-token": token}, cookies={"auth_sid": auth_sid})
        expected_values = ["email", "firstName", "firstName"]
